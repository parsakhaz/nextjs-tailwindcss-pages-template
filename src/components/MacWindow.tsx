...
export default function MacWindow({ children, className = '' }: MacWindowProps) {
  ...
  const extraStyles = macWindowConfig.variant === 'glassmorphic' ? {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        borderRadius,
        boxShadow: shadow === 'lg' ? '0 10px 30px -5px rgba(0, 0, 0, 0.3)' : undefined,
        ...extraStyles // Add extra styles
      }}
          ...
          <div className="flex gap-2 absolute">
            <motion.div
              className={`w-3 h-3 rounded-full ${
                variantStyles.buttonStyle === 'minimal' ? 'bg-white/20' : 'bg-[#ff5f57]'
              }`}
              whileHover={animations.buttonHover ? { scale: 1.1 } : undefined}
            />
            <motion.div
              className={`w-3 h-3 rounded-full ${
                variantStyles.buttonStyle === 'minimal' ? 'bg-white/20' : 'bg-[#febc2e]'
              }`}
              whileHover={animations.buttonHover ? { scale: 1.1 } : undefined}
            />
            <motion.div
              className={`w-3 h-3 rounded-full ${
                variantStyles.buttonStyle === 'minimal' ? 'bg-white/20' : 'bg-[#28c841]'
              }`}
              whileHover={animations.buttonHover ? { scale: 1.1 } : undefined}
            />
          </div>
