...
const CommandPaletteDemo: DemoComponent = () => {
	...
	const filteredCommands = commands.filter((cmd: Command) => cmd.title.toLowerCase().includes(searchTerm.toLowerCase()));
	...
	const {
		animations: { searchInputDelay, itemStaggerDelay, hoverTransitionDuration, slideInDistance },
	...
	return (
		<MacWindow className='max-w-2xl'>
			<div className='p-4'>
				<motion.div initial={{ opacity: 0, y: -slideInDistance }} animate={{ opacity: 1, y: 0 }} style={{ maxHeight, width }} className='relative'>
					{/* Search Input */}
					...
					<AnimatePresence>
								...
								{filteredCommands.map((command: Command, index: number) => (
									<motion.div
										...
										animate={{
											opacity: 1,
											x: 0,
											backgroundColor: selectedIndex === index ? 'rgba(255, 255, 255, 0.15)' : 'rgba(255, 255, 255, 0.05)',
										}}
										whileHover={{
                      ...
                      text-white/80 cursor-pointer'
									...
									>
										...
										{command.shortcut && <kbd className='text-white/40 font-mono text-sm'>{command.shortcut}</kbd>}
									</motion.div>
								))}
							</motion.div>
						)}
